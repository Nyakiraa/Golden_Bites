/**
 * Script to create Kuya Kim Cuisine account in Supabase
 * 
 * Usage: node scripts/create-kuyakim-account.js
 * 
...
async function createStallAccount(stallData) {
  try {
    ...
    if (userError) {
      console.warn('⚠️  Warning: Could not create user profile:', userError.message);
      console.warn('   This might be because the profile already exists (auto-created by trigger).');
    } else {
      console.log('✓ User profile created:', userRecord);
    }

    // Step 3: Create stall record in database
    console.log('Step 3: Creating stall record in database...');
...
// Kuya Kim Cuisine data
const kuyaKimCuisine = {
...
// Run the script
createStallAccount(kuyaKimCuisine)
  .then(() => {
    console.log('\n✨ Done!');
    process.exit(0);
  })
